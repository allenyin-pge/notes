...
Back from the Himalayas! Took a week to ramp back onto things, complete trainings, preparing for TIMP Bi-monthly presentation on Corrosion Outlier analysis again.

Now, back to TPD-ML modeling...

## Conversation with Gordon about data

Motivating question: What's the best way to compile data from different years, since PODS data have temporal consistency issue?

- MarinerDB serves as a good intermediate source for this.
- Prior to 2018, Gordon did spatial matching in ArcGIS.
- 2018 onward, the process was:
  - Generate interpolation tables, using a combination of attribute matching (engineering stationing, PFL feature number, route stationing match, etc) and spatial matching for those that don't match via attribute.
  - Use the interpolation tables and SQL scripts to do spatial joining

So I can grab previous years' MarinerDB and replicate the joining process with interpolation tables generated by Gordon to temporally compile all the years' data together. But how do I make sure things are ok?

> Look at th emap to compare the two years' line segments. In most cases it's fairly obvious based on the surrounding context, whether mapping moved the whole route, shifted the centereline, etc. Then compare the PFL feature numbers, etc.

> In some cases especially with shorter routes and those have been completely changed, there is no way to be sure.

In practice, the following steps are done:
1. GTGIS generate new PODS snapshot
2. Gordon ingest it into MarinerDB.
3. Gordon generate the interpolation table against previous year's MarinerDB or PODS data
4. Gordon run the workflow to do spatial + attribute matching.
5. Gordon runs risk modeling on the GDB.
6. Bill Boynton access the temporally matched GDB and do validation.
  - He only validates what's relevant to next few years' projects. Most of other pipes not validated by his team.
  - If found errors, update on their scoping database whether to adjust the project. And only spot-fixes for identified errors in the MarinerDB. Step 3-4 are not repeated.

Currently, 2018-2022 MarinerDB available on prod server: `pritinfdbsws079`.
- Anything older than 2018 on GDBS on SI drive, which has no interpolation tables.

Gordon used SQL script to migrate tables from 2022 to 2023 snapshot (`Restation_2022\_to\_2023.sql`)
- The same script need to be adjusted to apply to different tables.
- Roughly the same every year
- In practice only done for tables that contain historical data though (e.g. mitigation history)

# Week of 11/3/2024

## BDS training (Business Data Steward: Business data management training)

What is it? Why do I need to do it? What do I gain from it?
- For me: metadata maintenance...
- BDS: Dataset portfolio management
- People from an org pick whatever datasets to be the BDS of.

BDS main tools to monitor datasets
- Collibra: metadata management. Metadata pushes from Collibra into Foundry
- Foundry
- Data quality
- What to do? Monitor datasets, if we can't mitigate it ourselves, call someone for "data remediation project".

Collibra + Data quality might both become Informatica...

*Tangent*:Why not Foundry for everything (metadata management, analytics, and data quality)
- Snowflake is cheaper and easier to connect to other data sources..
- Everyday user: use snowflake to get the data
- Foundry is used by wildfire and Sumeet protects it.
  - Pricing structure is by cores and threads and expensive
- Right now keeping SAP, Snowflake, and Foundry

Training sessions:
- 11/12: Foundations

Gas specific training:
- Day 1 Foundry: 9-1130am
- Day 2 Collibra: 9-1130am

Let's wait till Feb. to do it with Satvinder.

Alec targeting these datasets:
- ILI pipe tally
- CIS Read
- Leakmaster
...
Foundry geospatial workflow is not working, bugs in their coordinate transformation tools. So now have to do all the spatial joins between different TPD factors, risks, and ILI in ArcGIS, which is hard to keep track of.

Jeremy suggests using the `<Risk>_Risk_percentile_Full_Spreadsheet` table that Gordon outputs, which contains what I want, for individual threats... but it may be enough already to join with ILI data.
- Ask Gordon's team to pull data together for me.
- Also ask Even for whatever data he's pulled before?

Push more about getting the county-construction spending.

- Try to start restationing process improvement again (version control, etc)

# 12/2/2024

All the variables I need for TPD.

- From Dig-in trackers:
```
'OBJECTID', 'Join_Count', 'TARGET_FID', 'JOIN_FID', 'TrackerID',
'Date_Added', 'Type_', 'Type_Detail', 'Source', 'TPD_Additional_Detail',
'LeakMaster_UniqueID', 'DIRT_UniqueID', 'CE_Unique_ID', 'CE_Type',
'Project_Name', 'Leak_Number', 'Leak_Type', 'Route', 'MP', 'City',
'Event_Date', 'Northing', 'Easting', 'Lat', 'Long_', 'Rupture',
'PreviousID', 'BUFF_DIST', 'ORIG_FID', 'sortid', 'route_1',
'beginstationseriesid', 'beginstationnum', 'endstationseriesid',
'endstationnum', 'Warning_Tape', 'LandUse', 'Third_Party_History',
'First_Second_Party_History', 'Cover_Depth', 'Pipe_Exposed',
'Dig_Without_811', 'Line_Markers', 'Structural_Encroachment',
'Feature_Type', 'PFL_Feature_Num', 'Install_Date', 'Seam_Type',
'Outside_Diameter', 'Wall_Thickness', 'Aerial_Patrol_Flights',
'Designed_Span', 'ForeignLineCrossing', 'Dent_Gouge_Damage_History',
'Future_Development', 'Repeat_Offender', 'One_Call_Tickets',
'Public_Awareness', 'Percent_SMYS', 'Operation_Area', 'Length_FT',
'MP1', 'MP2', 'avg MP', 'delta', 'validation_sample_ID',
'Shape_Length_1', 'Shape_Length', 'Shape_Area'
```

- Union of all columns:
```
{'RD_ResistancetoDeformation', 'delta', 'USATickets', 'Warning_Tape', 'Dent_Gouge_Damage_History', 'Shape_Area', 'BUFF_DIST', 'DI12_FirstSecondPartyDigIn', 'Aerial_Patrol_Flights', 'Shape_Length', 'StrucEncroachment', 'Easting', 'STPD_TPDScore', 'Third_Party_History', 'endstationnum', 'WarningTape', 'Northing', 'RR_RuptureRate_TPD', 'AF_FirstSecond', 'RepeatOffenders', 'Join_Count', 'Leak_Type', 'TrackerID', 'ForeignPipeline', 'Shape_Length_1', 'Future_Development', 'S12_B12_FirstSecondPartyDamageScore', 'Long_', 'route_1', 'RR_RuptureRate_FirstSecond', 'OBJECTID', 'Structural_Encroachment', 'route', 'Type_Detail', 'Designed_Span', 'ResistancetoDeformation', 'Lat', 'Line_Markers', 'endstationseriesid', 'Rupture', 'MP2', 'Cover_Depth', 'Source', 'Project_Name', 'PublicAwareness', 'Leak_Number', 'Operation_Area', 'CVN', 'AI_ActivityIndicator', 'Repeat_Offender', 'sortid', 'Event_Date', 'MP1', 'Seam_Type', 'Public_Awareness', 'Outside_Diameter', 'No811Patrol', 'DIRT_UniqueID', 'FutureDevelopment', 'Install_Date', 'AerialPatrol', 'Percent_SMYS', 'ROD', 'PreviousID', 'avg MP', 'Wall_Thickness', 'Dig_Without_811', 'beginstationnum', 'LandUse', 'CE_Unique_ID', 'PM_PreventativeMeasure', 'First_Second_Party_History', 'City', 'Route', 'ORIG_FID', 'JOIN_FID', 'DIW_ThirdPartyDigIn', 'Length (Miles)', 'Pipe_Exposed', 'TPD_Additional_Detail', 'CE_Type', 'Feature_Type', 'Date_Added', 'DepthofCover', 'Type_', 'ForeignLineCrossing', 'PFL_Feature_Num', 'LineMarker', 'beginstationseriesid', 'MP', 'One_Call_Tickets', 'TARGET_FID', 'AF_TPD', 'LeakMaster_UniqueID', 'LOF_TPD', 'OperatingScore', 'Length_FT', 'validation_sample_ID'}
```

- External dataset needed:
  - Construction spending per county (census burea)
  - Agricultural landuse (PGE) (California natural resources agency)
    - Actually this is in the `TPD_LU_Union_Dissolve` gdb layer for 2020
  - Gas usage (PGE Energy Data Request Portal)
  - Population (IRS)

- In ValidationGDB:
  - `timpleakrepair` layer contains Dig-in tracker data.

# 12/4/2024

Info from Gordon about what data to join for TPD:
- `TPD_Risk_Results` table from SQL database. To get it in a format to associate with ILI data, have two options:
  - Join with `Pipesegment_linear`: but since TPD table have ~~higher resolution~~ longer segments, then there's ~~many-to-one~~ one-to-many relationship between `TPD_Risk_Results` record and `Pipesegment_linear` segments.
  - Spatialize `TPD_Risk_results` using "each year's centerline", with ArcGIS linear referencing "Make Route Event Layer" tool, then store the event layer as a feature class.


# 1/9/2025

Got all these spatial joins stuff figured out, documenting the detailed steps so I can repeat later.

Example using 2022 data to illustrate

## Overall steps:

1. Spatialize TPD results with centerline data
2. Join the spatialized results to cleaned ILI data.

### Spatializing TPD results

1. Obtain each year's TPD LOF results table, which contains all the input factors used in the TPD model. This is convenient because it contains the proper attribution of the different covariates with the correct pipe segment.
    - These are stored in each year's Mariner Database tables:
      - pritinfdbsws079: contains 2019 to 2022 MarinerDB archives.
      - tsitinfdbsws010.MarinerDB: contains 2023 tables
    - The table is called "TPD_Risk_Results"
    - Action:
      - Create database connection to `pritinfdbsws079` in ArcMap,
      ![db_connect](./assets/copy_TPD_risk_results.png)
      - Export the table "TPD_Risk_Results" to the local copy of 2022 validation.gdb
      ![Export table](./assets/export_TPD_risk_results.png)

2. Get the centerlines layers from QRAD database -- copy the layer to the corresponding local validation geodatabase
    -2019: `\\rcnas01-smb\sysintegrity-fs01\SysIntegrity\RiskMgmt\SI\SME_Tables_RiskAnalysis\QRAD_2019\Snapshot\RiskPublish.gdb\Centerline_070819`
    - 2020: `\\rcnas01-smb\sysintegrity-fs01\SysIntegrity\RiskMgmt\SI\SME_Tables_RiskAnalysis\QRAD_2020\DataQuality\RiskSnapshot.gdb\Centerline_060720`
    - 2021: `\\rcnas01-smb\sysintegrity-fs01\SysIntegrity\RiskMgmt\SI\SME_Tables_RiskAnalysis\QRAD_2021\DataQuality\RiskSnapshot_restored.gdb\Centerline_06072021`
    - 2022: `\\rcnas01-smb\sysintegrity-fs01\SysIntegrity\RiskMgmt\SI\SME_Tables_RiskAnalysis\QRAD_2022\DataQuality\RiskSnapshot.gdb\centerline_06082022`
    - 2023: `\\rcnas01-smb\sysintegrity-fs01\SysIntegrity\RiskMgmt\SI\SME_Tables_RiskAnalysis\QRAD_2023\DataQuality\RiskSnapshot.gdb\Centerline_06092023`
    ![centerline_layer](./assets/copy_centerline_layer.png)

3. Spatialize the TPD_Risk_Results table with the centerline using ArcGIS linear referencing `Make Route Event Layer` tool, and store resulting layer as a feature class.

![tool_access](./assets/make_route_event_layer_tool.png)

In the tool's settings, the "route" is the centerline layer, while the events are the TPD_Risk_Results. The risk results data will be treated as route "events".

Also note in the centerline layer attributes, the column name is "ROUTE" while the column name is "route" in TPD_Risk_Results table.

![using_tool](./assets/make_route_event_layer_settings.png)

The results then look like the following -- most of the centerlines overlap with the spatialized events, but it looks like not all centerlines have available TPD_Risk_Results, which is probably due to small distribution pipelines

![spatialized_TPD_Risk_Results](./assets/TPD_Risk_Results_spatialized.png)

Now we can save the resulting layer as a shape-file, using the `Right-Click > Data > Export` menu. Save it in `ModelValidation2022/ILI_data/TPD_Risk_Results_SpatializedBy_Centerline.shp` for example.

### Spatialize ILI data

`cleaned_ILI_2022` shape file is already in `ModelValidation2022/ValidationData.gdb`, this was created following instructions in [9/27/2023](#09272023) step 1- 3.

Now spatialize `cleaned_ILI_2022` by spatial-joining it with the centerlines layer

![spatial_join_ILI_centerlines](./assets/spatial_join_ILI_centerlines.png)

### Spatial-join Spatialized TPD_Risk_Results to Spatialized ILI data

Now we want to spatial join the TPD_Risk_Results together with ILI data so we can compare the corresponding measurements.

The spatialized TPD_Risk_Results is saved in `ModelValidation2022/ValidationData.gdb/TPD_Risk_Results_SpatializedBy_centerlines`, the ILI layer is saved in `ModelValidation2022/ValidationData.gdb/cleaned_ILI2022_joinedFrom_centerlines`.

In spatial-join, JOIN FEATURES are appended to TARGET feature. Since the ILI event has finer resolution, then the TARGET feature should be the ILI data. This means in the final output, different ILI rows found on a segment would have the same TPD_Risk_Results conditions for that segment -- this is a one-to-many relationship, but the Spatial-join setting should have __JOIN ONE To ONE__ since it specifies the join feature (TPD_RISK) to target feature (ILI).

Looking at both layers plotted before spatial-join, it looks like they overlap well

![TPD_ILI_overlapped](./assets/TPD_ILI_overlapped.png)

But spatial-joining them with intersect or closest ends up with either:
1. Many ILI rows has null attributes appended corresponding to the TPD columns
2. Very small number of ILI rows result in the final joined layer.

The reason is because the ILI points and TPD lines don't actually overlap, zoomed in we see:

![ILI_TPD_offset](./assets/ILI_TPD_offset.png)

This means we need to use spatial-join using "within a distance of" setting. But how to find the radius?
- Go to "Selection" > "Select by Location"
- In the menu, set "Target Layer" to be the "cleaned_ILI2022_spatializedBy_centerlines" layer, and "Source layer" as the "TPD_Risk_Results_spatializedBy_centerlines" layer
- Select "Spatial selection method for target layer feature" as "Within distance", set different radius there and see what number is needed to select most of the ILI points.
- This ended up being 15ft.

Now we run spatial join again, but the join method as "Within distance" of 15ft, target is the ILI layer, join feature is the TPD_Risk_Results layer.

![spatial_join_ILI_TPD_Risk_Results](./assets/within_distance_spatial_join.png)

The resulting feature class is saved to `ModelValidation2022/ValidationData.gdb/TPD_Risk_Results_SpatializedBy_centerlines_JoinedTo_ILI_JoinedFrom_centerlines`

### Examine the outputs and export file for analysis

Now the final layer looks like below:

![TPD_ILI_spatial_joined](./assets/TPD_ILI_spatial_joined.png)

Note that each row in ILI now has corresponding TPD risk results attributes!
